# Возьмем какое-нибудь натуральное число n. Будем изменять его следующим образом: если число четное,
# то разделим его на 2, если нечетное, прибавим 1. После нескольких таких изменений мы всегда получаем число 1.
# Например, из числа 11 получается число 12, затем 6, 3, 4, 2 и наконец 1. Таким образом, для получения 1 из 11
# нужно проделать 6 изменений.
# По заданному натуральному числу найти количество его изменений до получения 1.
# Входные данные
# Одно натуральное число n (1 ≤ n ≤ 109).
# Выходные данные
# Вывести количество изменений числа n до получения 1.

n = int(input())
count = 0
while n != 1:
    if n%2 == 0:
        n/=2
        count += 1
    else:
        n+=1
        count += 1
print(count)


